...
app.get('/components/:name', (req, res) => {
  const safeName = req.params.name.replace(/[^a-z0-9-]/gi, '');
  const file = path.join(root, 'docs', 'components', `${safeName}.html`);
...
app.listen(PORT, () => {
  console.log(`MBrutals v1.5.6 docs running on http://localhost:${PORT}`);
});
